...
class PreQueryListener implements ApplicationListener<PreQueryEvent> {
    ...
    void onApplicationEvent(PreQueryEvent event) {
        ...
        if(LogicalDeleteDomainClassEnhancer.classProperties[domainClass]) {
            if(!event.query.session.getSessionProperty(LogicalDeleteDomainClassEnhancer.PHYSICAL_SESSION)) {
                event.query.eq(
                        LogicalDeleteDomainClassEnhancer.classProperties[domainClass].property,
                        !LogicalDeleteDomainClassEnhancer.classProperties[domainClass].deletedState
                )
            }
        }
    }
